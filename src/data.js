...
const leaguesList = (responseText, sport) => {
  ...
  if (listObject.length > 0 && isNotLeague.length > 0) {
    allLeagues.push({
      [isNotLeague[0].childNodes[0].rawText.replace(
        ' Current Leagues',
        '',
      )]: [],
    });
    listObject.map(x => {
      if (x.childNodes[1].childNodes && x.childNodes[1].childNodes[3]) {
        allLeagues.push({
          [x.childNodes[1].childNodes[3].childNodes[0].rawText]: [],
        });
      }
      const arra =
        allLeagues[allLeagues.length - 1][
          Object.keys(allLeagues[allLeagues.length - 1])[0]
        ];
      if (x.childNodes[1].childNodes[0].rawText.length > 7) {
        ...
        arra.push({
          ...
          division: leagueSplit[2]
            ? leagueSplit
                .slice(2, leagueSplit.length)
                .toString()
                .trim()
                .replace(/amp;/g, '')
            : leagueSplit[1].trim(),
      ...
      if (x.childNodes[3]) {
        Object.assign(arra[arra.length - 1], {
          fixtures: x.childNodes[3].childNodes[0].rawAttrs
            .replace('class="LFixtures" href=', '')
            .replace(/"/g, '')
            .replace(/amp;/g, ''),
        });
      }
      if (x.childNodes[5]) {
        Object.assign(arra[arra.length - 1], {
          standings: x.childNodes[5].childNodes[0].rawAttrs
            .replace('class="LStandings" href=', '')
            .replace(/"/g, '')
            .replace(/amp;/g, ''),
        });
      }
...
const allFixturesAndStandings = (responseText, sport) => {
  ...
  const fixturesUrls = keysSorted(
    distribution(
      list.map(
        l => `${l.fixtures.replace(/([^&]*)$/g, '').trim()}Level=Season`,
      ),
    ),
  )[0];
...
const teamsList = responseText => {
  ...
  const list = listObject.map(i => ({
    team: i.childNodes[1].childNodes[0].childNodes[0].rawText.replace(
      /amp;/g,
      '',
    ),
    teamUrl: i.childNodes[1].childNodes[0].rawAttrs
      .replace('HREF=', '')
      .replace(/"/g, '')
      .replace(/amp;/g, ''),
  ...
  listObject.map(i => {
    ...
    i.childNodes.map(j => {
      ...
      return j.childNodes.map(k => {
        if (k.rawAttrs === 'class="FDate"') {
          matches.time = k.childNodes[0].rawText;
        }
        if (k.rawAttrs === 'class="FPlayingArea"') {
          matches.court = k.childNodes[0].rawText;
        }

        if (k.rawAttrs === 'class="FHomeTeam"') {
          matches.homeTeam = k.childNodes[0].childNodes
            ? k.childNodes[0].childNodes[0].rawText.replace(/amp;/g, '')
            : '';
        }
        if (k.rawAttrs === 'class="FHomeTeam"') {
          matches.homeTeamUrl = k.childNodes[0].childNodes
            ? k.childNodes[0].rawAttrs
                .replace('href=', '')
                .replace(/"/g, '')
                .replace(/amp;/g, '')
            : '';
        ...
        if (k.rawAttrs === 'class="FAwayTeam"') {
          matches.awayTeam = k.childNodes[0].childNodes
            ? k.childNodes[0].childNodes[0].rawText.replace(/amp;/g, '')
            : '';
        }
        if (k.rawAttrs === 'class="FAwayTeam"') {
          matches.awayTeamUrl = k.childNodes[0].childNodes
            ? k.childNodes[0].rawAttrs
                .replace('href=', '')
                .replace(/"/g, '')
                .replace(/amp;/g, '')
  ...
  listObject.map(i => {
    ...
    i.childNodes.map(j => {
      ...
      return j.childNodes.map(k => {
        ...
        if (k.rawAttrs === 'class="FHomeTeam"') {
          matches.homeTeam = k.childNodes[0].childNodes[0]
            ? k.childNodes[0].childNodes[0].rawText.replace(/amp;/g, '')
            : '';
        }
        if (k.rawAttrs === 'class="FHomeTeam"') {
          matches.homeTeamUrl = k.childNodes[0].childNodes[0]
            ? k.childNodes[0].rawAttrs
                .replace('href=', '')
                .replace(/"/g, '')
                .replace(/amp;/g, '')
        ...
        if (k.rawAttrs === 'class="FScore"') {
          matches.score = k.childNodes[0].childNodes
            ? k.childNodes[0].childNodes[0]
              ? k.childNodes[0].childNodes[0].rawText.replace(/\n\t/g, '')
              : 'no result'
            : 'vs';
        }
        if (k.rawAttrs === 'class="FScore"') {
          matches.scoreUrl =
            k.childNodes[0].childNodes &&
            k.childNodes[0].rawAttrs.includes('href')
              ? k.childNodes[0].rawAttrs
                  .replace(
                    'title="Click on score to open scoresheet" href="javascript: var o = window.open( \'',
                    '',
                  )
                  .replace("' )", '')
                  .replace('\\', '')
        ...
        if (k.rawAttrs === 'class="FAwayTeam"') {
          matches.awayTeam = k.childNodes[0].childNodes[0]
            ? k.childNodes[0].childNodes[0].rawText.replace(/amp;/g, '')
            : '';
        }
        if (k.rawAttrs === 'class="FAwayTeam"') {
          matches.awayTeamUrl = k.childNodes[0].childNodes[0]
            ? k.childNodes[0].rawAttrs
                .replace('href=', '')
                .replace(/"/g, '')
                .replace(/amp;/g, '')
...
const playerList = responseText => {
  ...
  const allStats = listObject.map(i => {
    ...
    if (i.childNodes[2]) {
      ...
      stats.contributionAverage =
        i.childNodes[12 - downOne].childNodes[0].rawText;
    } else {
      stats.played = 'N/A';
      stats.runs = 'N/A';
...
const divisionPlayerList = responseText => {
  ...
  const allStats = listObject.map(i => {
    ...
    if (i.childNodes[4]) {
      ...
      stats.contributionAverage =
        i.childNodes[14 - downOne].childNodes[0].rawText;
    } else {
      stats.team = '';
      stats.division = '';
      stats.played = 'N/A';
